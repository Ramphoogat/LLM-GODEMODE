...
export function SettingsModal({ onClose }: SettingsModalProps) {
  ...
  return (
    ...
    >
      ...
      {/* Modal — constrained to viewport, never overflows */}
      <div
        className="relative flex flex-col w-full rounded-xl shadow-2xl overflow-hidden"
        style={{
          maxWidth: '860px',
      ...
      >
        ...
        >
          <div className="flex items-center gap-2">
            ...
            >
              G0DM0D<span style={{ display: 'inline-block', transform: 'scaleX(-1)' }}>E</span> Settings
            </h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex flex-1 min-h-0">
          ...
          <nav
            className="flex-shrink-0 overflow-y-auto py-2 px-1.5"
            style={{
              width: '180px',
          ...
          >
            {([
              { id: 'api',          icon: <Key className="w-3.5 h-3.5" />,             label: 'API Key' },
              { id: 'prompt',       icon: <FileText className="w-3.5 h-3.5" />,        label: 'System Prompt' },
              { id: 'autotune',     icon: <SlidersHorizontal className="w-3.5 h-3.5" />, label: 'AutoTune' },
              { id: 'appearance',   icon: <Palette className="w-3.5 h-3.5" />,         label: 'Appearance' },
              { id: 'privacy',      icon: <Shield className="w-3.5 h-3.5" />,          label: 'Privacy' },
              { id: 'liquid',       icon: <Droplets className="w-3.5 h-3.5" />,        label: 'Liquid' },
              { id: 'parseltongue', icon: <Skull className="w-3.5 h-3.5" />,           label: 'Parseltongue' },
              { id: 'stm',          icon: <Zap className="w-3.5 h-3.5" />,             label: 'STM Modules' },
              { id: 'memory',       icon: <Sparkles className="w-3.5 h-3.5" />,        label: 'Memory' },
              { id: 'ultraplinian', icon: <Swords className="w-3.5 h-3.5" />,          label: 'ULTRAPLINIAN' },
              { id: 'consortium',   icon: <Users className="w-3.5 h-3.5" />,           label: 'CONSORTIUM' },
              { id: 'plan',         icon: <Crown className="w-3.5 h-3.5" />,           label: 'Plan' },
              { id: 'data',         icon: <Download className="w-3.5 h-3.5" />,        label: 'Data' },
  ...
  return (
    <button
      ...
      style={{
        ...
        textShadow: active ? '0 0 6px var(--primary)' : 'none',
        marginBottom: '2px',
      }}
        ...
        <>
          ...
          {previewParams && (
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {(Object.entries(previewParams) as [string, number][]).map(
                  ([key, value]) => (
                    <div
                      key={key}
          ...
          <div>
            ...
            <div className="space-y-3">
              {(Object.keys(PARAM_META) as string[]).map((paramKey) => {
                          ...
                          onClick={() => {
                            if (isOverridden) {
                              ...
                              const defaultVal = previewParams
                                ? previewParams[paramKey as keyof AutoTuneParams]
                                : (meta.min + meta.max) / 2
...
function AppearanceTab() {
  ...
  const themes: {
  ...
  }[] = [
    {
      ...
      secondary: '#008f11',
      accent: '#003b00',
      desc: 'Classic green terminal on dark void',
    },
    {
      id: 'hacker',
      name: 'Hacker',
      ...
      secondary: '#ff8c00',
      accent: '#1a0000',
      desc: 'Red-and-orange hacker aesthetic',
    },
    {
      id: 'glyph',
      name: 'Glyph',
      ...
      secondary: '#0f3460',
      accent: '#0f3460',
      desc: 'Deep navy with electric crimson',
    },
    {
      id: 'minimal',
      name: 'Minimal',
      ...
      secondary: '#737373',
      accent: '#e5e5e5',
      desc: 'Clean white with charcoal ink',
    },
    {
      id: 'solar',
      name: 'Solar',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div>
        ...
        <p style={{ fontSize: '12px', color: 'var(--text-dim)', marginBottom: '4px' }}>
          Choose your aesthetic — persists across sessions automatically.
        </p>
        {saved && (
          <span style={{
            display: 'inline-flex',
          ...
          return (
            <button
              ...
              style={{
                ...
                background: isActive ? `${t.primary}10` : 'var(--bg)',
                cursor: 'pointer',
                textAlign: 'left',
                transition: 'all 0.2s',
              ...
              onMouseEnter={e => {
                if (!isActive) (e.currentTarget as HTMLElement).style.borderColor = t.primary
              }}
              onMouseLeave={e => {
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
                ...
                <span style={{
                  fontWeight: 700,
                  fontSize: '13px',
                  color: isActive ? t.primary : 'var(--text)',
                }}>
                  {t.name.toUpperCase()}
                </span>
              </div>

# G0DM0DƎ: A Modular Research Framework for Evaluating LLM Robustness Through Adaptive Sampling, Input Perturbation, and Multi-Model Safety Assessment
...
G0DM0DƎ addresses these research needs through a modular framework operating entirely at inference time, requiring no model fine-tuning, no weight access, and no provider-specific APIs beyond the standard chat completion interface. The system is implemented in approximately 3,300 lines of TypeScript and operates through the OpenRouter multi-model gateway.
...
G0DM0DƎ is designed as a modular safety research pipeline where each component can be independently enabled, configured, and studied in isolation. The framework processes a research query through up to five stages, each independently toggleable:
...
A key challenge for empirical AI safety research is reproducibility: understanding not just *what* a framework does, but *how it is used* at scale, *which* steering primitives researchers activate, and *what* failure modes arise in practice. G0DM0DƎ addresses this through a three-tier data collection architecture that captures progressively richer information at each level, with privacy guarantees enforced by construction at every tier.
...
We evaluate all five G0DM0DƎ modules through computational experiments. All evaluation scripts are reproducible and included in the repository at `research/eval_*.ts`. Experiments run on the actual engine code — no simulation or reimplementation.
...
**Three-tier data collection and privacy guarantees.** G0DM0DƎ implements a three-tier data collection architecture (Section 3.8) with explicit, verifiable privacy guarantees at each tier. We enumerate these guarantees here for ethical review:
